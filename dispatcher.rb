module Arisa
    ...
    class Dispatcher
        ...
        attr_reader :crash_modules, :created_crash_modules, :issue_modules, :updtd_modules, :query_modules
        ...
        def process_issues(client)
            ...
            issue_pool.eat(:created).each do |issue|
                ...
                process_created_crash_reports(client, issue)
            end
        ...
        def process_created_crash_reports(client, issue)
            reports = IssueParser.new(issue).reports
            return if reports.empty?
            created_crash_modules.each { |target| target.process(client, issue, reports.dup) }
        end
